...
$(document).ready(function() {
	...
	if(!badIE){
		...
		$('a[href^=#]').bind('click touchstart',function(){hash%20=%20$(this).attr('href');
	...
	win.bind('load',function(){
		...
		var sectionselector = 'section';//Documentation%20has%20subcategoriesif(nav.find('ol').length){
		...
		win.bind('scroll',function(event){
			...
			timeout = setTimeout(function(){
				...
				//iDeviceNotOS4s don't%20know%20the%20fixed%20property%20so%20we%20fake%20itif(iDeviceNotOS4){sidebar.css({height:document.height});logo.css({'margin-top':pos});
				}
	...
	function goTo(hash,changehash){
		...
		if(changehash !== false){
			var l = location;
			location.href = (l.protocol+'//'+l.host+l.pathname+'#!/'+hash.substr(1));}win.bind('hashchange', hashchange);
	}
	...
	function activateNav(pos){
		...
		for(var i=sectionscount;i>0;i--){
			if(sections[i-1].pos <= pos+offset){
				...
				current.addClass('current');parent%20=%20current.parent().parent();next%20=%20current.next();hasSub%20=%20next.is('ol');isSub%20=%20!parent.is('#documenter_nav');nav.find('ol:visible').not(parent).slideUp('fast');
				if(isSub){
					parent.prev().addClass('current');
					parent.stop().slideDown('fast');
				}else if(hasSub){
					next.stop().slideDown('fast');}win.bind('hashchange', hashchange);
				break;
			};
		}	
	}
